...
// nestedFunction() is nested inside of myFunction() which is where the variable internal is declared.
//The outer function is within the nested functions lexical environment which is why it can be accessed.
// The variable external can also be accessed by nested function, however, if internal was declared in the nested function, myFunction() would not be able to access it.
...
function animalNames(arr) {
  let nameNSciName = [];
  arr.forEach(obj => nameNSciName.push(`name: ${obj.animal_name}, scientific: ${obj.scientific_name}`));
  return nameNSciName;
}
...
function lowerCaseNames(arr) {
  const lowerCase = arr.map(object => object.animal_name.toLowerCase());
  return lowerCase;
}
...
function lowPopulationAnimals(arr) {
  const lowPopArr = arr.filter(object => object.population < 5);
  return lowPopArr;
}
...
function USApop(arr) {
  const totalPop = arr.reduce((sum, i) => sum += i.population,0);
  return totalPop;
}
